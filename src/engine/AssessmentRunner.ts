...
export interface AssessmentResult { status: 'passed' | 'failed' | 'unable_to_verify'; passed: number; total: number; logs: string[]; }

export class AssessmentRunner {
  constructor(private readonly engine: InBrowserLinuxEngine) {}
  public runCTestSuite(sourceCode: string): AssessmentResult { return this.evaluateSuite(sourceCode, 'c', [
  ...
  public runJavaTestSuite(sourceCode: string): AssessmentResult { return this.evaluateSuite(sourceCode, 'java', [
  ...
  private evaluateSuite(code: string, language: 'c' | 'java', suite: TestCase[]): AssessmentResult {
    let passed = 0; let unverifiable = false; const logs: string[] = []; const started = performance.now();
    for (const tc of suite) {
      const output = this.engine.executeGeneralCode(code, language, { [tc.injectedVar.name]: tc.injectedVar.value });
      const executionFailed = /^\x1b\[31m\[Transpiler Error\]:/.test(output) || output.includes('[Execution Error]');
      if (executionFailed) { unverifiable = true; logs.push(`<div class="test-unknown">? [Unable to verify] ${tc.description}: execution failed</div>`); continue; }
      if (output.includes(tc.expectedSubstring)) { passed++; logs.push(`<div class="test-pass">✓ [Passed] ${tc.description}</div>`); }
      else logs.push(`<div class="test-fail">✗ [Failed] ${tc.description} (expected output was not produced)</div>`);
    }
    ...
    void StorageService.saveProgress(`default-${language}`, passed, status === 'passed', status, logs);
